...
fn get_solution_tree(mut cache: Trie) -> (Tree, Trie) {
    return if !cache.is_done || cache.root.data.test == <usize>::MAX {
...
fn get_sub_tree(parent: Vec<Item>, mut cache: Trie) -> (Trie, Tree) {
    ...
    let parent_node_data = parent_node.data.clone();
